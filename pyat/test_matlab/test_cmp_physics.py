...
def test_ohmi_envelope(engine, lattices):
    ...
    # Comparison
    assert_close(damping_times, _py_data(ml_params['dampingtime']),
                 atol=1.e-6, rtol=1.e-3)
    assert_close(py_beamdata.mode_emittances, _py_data(ml_emit0['modemit']),
                 atol=1.e-20, rtol=2.e-3)
...
def test_parameters(engine, lattices, dp):
    ...
    assert_close(py_length, ml_length, rtol=1.E-8)
    ...
    assert_close(py_mcf, ml_mcf, rtol=1.E-8)
