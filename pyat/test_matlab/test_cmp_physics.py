...
def test_find_orbit4(engine, ml_lattice, py_lattice, dp, refpts):
    ...
    if refpts is None:
        ...
        py_orb4 = physics.find_orbit4(py_lattice, dp, refpts)
    else:
        ...
        numpy.testing.assert_almost_equal(numpy.asarray(ml_orbit4), py_orbit4[:4, :], decimal=8)
...
def test_find_m44(engine, ml_lattice, py_lattice, dp, refpts):
    ...
    if refpts is None:
        ...
        py_m44 = physics.find_m44(py_lattice, dp)
    else:
        ...
        # Matlab wrongly squeezes ml_stack if len(refpts)==1
        numpy.testing.assert_almost_equal(py_mstack, numpy.asarray(ml_mstack).reshape((4, 4, len(refpts))), decimal=8)
...
def test_linopt(engine, ml_lattice, py_lattice, dp, refpts):
    ...
    def compare_lindata(py_data, ml_data, decimal=8):
        ...
        if refpts is None:
            ...
            # Python call
            py_lindata0, py_nu, py_xsi = physics.linopt(py_lattice, 0.0, get_chrom=True)
