...
def test_find_syncorbit(engine, lattices, ct):
    ...
    # Matlab call
    ml_orbit5, ml_orb5 = engine.findsyncorbit(ml_lattice, ct,
                                              _ml_refs(refpts, nelems),
                                              nargout=2)
    ml_orbit5 = numpy.rollaxis(numpy.asarray(ml_orbit5), -1)
    # Comparison
    assert_close(py_orb5, _py_data(ml_orb5), atol=1.e-12, rtol=0)
    assert_close(py_orbit5[:, :5], ml_orbit5, atol=1.e-12, rtol=0)
...
@pytest.mark.parametrize('lattices',
                         [pytest.lazy_fixture('dba'),
                          pytest.lazy_fixture('hmba')])
def test_find_orbit4(engine, lattices, dp):
    py_lattice, ml_lattice, _ = lattices
    nelems = len(py_lattice)
    nrefs = nelems+1
    refpts = range(nrefs)
...
def test_find_orbit6(engine, lattices):
    py_lattice, ml_lattice, _ = lattices
    nelems = len(py_lattice)
    nrefs = nelems+1
    refpts = range(nrefs)
    ...
    # Matlab call
    ml_orbit6, ml_orb6 = engine.findorbit6(ml_lattice, _ml_refs(refpts, nelems),
                                           nargout=2)
...
@pytest.mark.parametrize('lattices',
                         [pytest.lazy_fixture('dba'),
                          pytest.lazy_fixture('hmba')])
def test_find_m44(engine, lattices, dp):
    py_lattice, ml_lattice, _ = lattices
    nelems = len(py_lattice)
    nrefs = nelems+1
    refpts = range(nrefs)
    morbit = engine.zeros(6, 1)
    porbit = numpy.zeros((6, 1))
    ...
    ml_mstack = numpy.rollaxis(numpy.asarray(ml_mstack).reshape((4, 4,
                                                                 nrefs)), -1)
    # Comparison
    assert_close(py_m44, numpy.asarray(ml_m44), atol=1.e-11, rtol=0)
    assert_close(py_mstack, ml_mstack, atol=1.e-11, rtol=0)


@pytest.mark.parametrize('lattices',
                         [pytest.lazy_fixture('hmba_rad')])
def test_find_m66_orb(engine, lattices):
    py_lattice, ml_lattice, _ = lattices
    nelems = len(py_lattice)
    nrefs = nelems+1
    refpts = range(nrefs)
...
@pytest.mark.parametrize('lattices',
                         [pytest.lazy_fixture('hmba_rad')])
def test_find_m66(engine, lattices):
    py_lattice, ml_lattice, _ = lattices
    nelems = len(py_lattice)
    nrefs = nelems+1
    refpts = range(nrefs)
    ...
    ml_m66, ml_mstack = engine.findm66(ml_lattice, _ml_refs(refpts, nelems),
                                       nargout=2)
    assert_close(py_m66, numpy.asarray(ml_m66), atol=1e-7, rtol=0)
