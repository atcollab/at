...
t_rev = ring.circumference/clight
h = 992
freqres = clight*h/ring.circumference
ring.set_rf_frequency(freqres)

ring.set_cavity_phase()
fring,_ = at.fast_ring(ring)
fring[0].PassMethod = 'CavityPass'
fring[0].TimeLag = 0
...
detuneHz = -5e4
frequency = freqres + detuneHz
qfactor = 4500
rshunt = 6e6
bucket_size = clight/freqres
current = 0.1 #A

welem = LongResonatorElement('wake', ring, srange, frequency, qfactor, rshunt, Nturns=wturns, Nslice=10)
...
mcf = at.get_mcf(ring.radiation_off(copy=True))
eta = mcf - 1/gamma**2 
...
numer = m_e * clight**2 * 2 * gamma * t_rev**2 * 2 * np.pi * fs
