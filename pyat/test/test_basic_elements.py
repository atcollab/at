...
def test_element_string_ordering():
    d = elements.Drift("D0", 1, attr=np.array(0))
    assert d.__str__() == (
        "Drift:\n       FamName: D0\n        Length: 1.0\n"
...
def test_insert_into_drift():
    ...
    el_list = drift.insert([(0.0, None), (0.3, None), (0.7, None), (1.0, None)])
    assert len(el_list) == 3
    np.testing.assert_almost_equal([e.Length for e in el_list], [0.3, 0.4, 0.3])
    # Test normal insertion
    el_list = drift.insert([(0.3, monitor), (0.7, quad)])
    assert len(el_list) == 5
    np.testing.assert_almost_equal(
        [e.Length for e in el_list], [0.3, 0.0, 0.25, 0.3, 0.15]
    )
    # Test insertion at either end produces -ve length drifts
    el_list = drift.insert([(0.0, quad), (1.0, quad)])
    assert len(el_list) == 5
    np.testing.assert_almost_equal(
        [e.Length for e in el_list], [-0.15, 0.3, 0.7, 0.3, -0.15]
    )
...
def test_correct_dimensions_does_not_raise_error(rin, func):
    ...
    rin = np.zeros((6,))
    func([], rin, 1)
    rin = np.array(np.zeros((6, 2), order="F"))
    func([], rin, 1)
...
def test_dipole_bend_synonym(rin, dipole_class, func):
    ...
    if func == element_track:
        ...
        func(b, rin)
    rin_expected = np.array([1e-6, 0, 0, 0, 0, 1e-7]).reshape((6, 1))
    np.testing.assert_almost_equal(rin, rin_expected)
    assert b.K == 0.0
    b.PolynomB[1] = 0.2
    assert b.K == 0.2
    b.K = 0.1
...
def test_marker(rin, func):
    ...
    if func == element_track:
        ...
        func(m, rin)
    np.testing.assert_equal(rin, rin_orig)
...
def test_drift_divergence(rin, func):
    ...
    if func == element_track:
        ...
        func(d, rin)
    # results from Matlab
    rin_expected = np.array([1e-6, 1e-6, -2e-6, -2e-6, 0, 2.5e-12]).reshape(6, 1)
    np.testing.assert_equal(rin, rin_expected)
...
def test_drift_two_particles(rin, func):
    d = elements.Drift("drift", 1.0)
    assert d.Length == 1
    two_rin = np.array(np.concatenate((rin, rin), axis=1), order="F")
    ...
    if func == element_track:
        ...
        func(d, two_rin)
    # results from Matlab
    p1_expected = np.array(two_rin_orig[:, 0]).reshape(6, 1)
    p2_expected = np.array([1e-6, 1e-6, -2e-6, -2e-6, 0, 2.5e-12]).reshape(6, 1)
    two_rin_expected = np.concatenate((p1_expected, p2_expected), axis=1)
    np.testing.assert_equal(two_rin, two_rin_expected)
...
def test_quad(rin, func):
    ...
    expected = (
        np.array(
            [0.9210610203854122, -0.3894182419439, 0, 0, 0, 0.0000000103303797478]
        ).reshape(6, 1)
        * 1e-6
    )
    np.testing.assert_allclose(rin, expected)
    assert q.K == 1
    q.PolynomB[1] = 0.2
    assert q.K == 0.2
    q.K = 0.1
...
def test_m66(rin, n, func):
    ...
    if func == element_track:
        ...
        func(m66, rin)
    expected = (
        np.array([m[0, n], m[1, n], m[2, n], m[3, n], m[4, n], m[5, n]]).reshape(6, 1)
        * 1e-6
    )
    np.testing.assert_equal(rin, expected)


@pytest.mark.parametrize("func", (element_track, element_pass, internal_epass))
def test_corrector(rin, func):
    c = elements.Corrector("corrector", 0.0, np.array([0.9, 0.5], dtype=np.float64))
    assert c.Length == 0
    rin[0, 0] = 1e-6
    rin_orig = rin.copy()
    rin_orig[1] = 0.9
