...
@pytest.mark.parametrize(
    "lattice", ["hmba_lattice", "noringparam_lattice"],
)
@pytest.mark.parametrize('refpts', ([145], [20], [1, 2, 3]))
...
def test_linopt(dba_lattice, refpts):
    ...
    obs = lindata[-1]
    assert_close(
        tune, [0.355804633927360, 0.488487169156598], rtol=1e-8
    )
    assert_close(
        chrom, [-3.428312247995742, -1.597924047969101], rtol=2e-4
    )
    ...
    assert_close(
        obs['beta'],
        [2.784841119739221, 6.460251554763623],
        rtol=5e-8, atol=1e-12
    )
    assert_close(
        obs['mu'],
        [2.235586452367286, 3.069255403991328],
...
def test_ohmi_envelope(request, lattice, refpts):
    ...
    assert_close(
        beamdata['tunes'],
[3.8156301859416669e-01, 8.5437639666920928e-01, 1.0906043887838406e-04],
         rtol=2e-6
    )

    assert_close(
        beamdata['damping_rates'],
[1.0082255910132057e-05, 6.5786188730294989e-06, 9.6535991636061425e-06],
         rtol=2e-6
    )

    assert_close(
        beamdata['mode_matrices'], numpy.array([
...
[1.3203595323533702e-10, 2.5462331612270083e-37, 2.8586905643787888e-06],
        atol=1e-9
    )
    ...
    assert_close(
        obs['r66'],
        numpy.array([
        [ 9.1365311942510863e-10, -3.4825078477958661e-15,  6.3211978321263912e-25,
          1.9795001676444935e-25,  1.5077612072840898e-09, -1.2011884887563537e-15],
        [-3.4825078478107073e-15,  1.9135586827896934e-11, -1.0575047753076275e-28,
         -3.2849211804285519e-29, -2.5110829486802159e-13, -1.2870493161486989e-13],
        [ 6.3211978321263912e-25, -1.0575047753076703e-28,  6.6159310088771835e-37,
          4.9450770495305794e-38,  3.6604851278961747e-22, -4.5428188275174624e-23],
        [ 1.9795001676444933e-25, -3.2849211804289672e-29,  4.9450770495305805e-38,
          9.1814623972342119e-38,  1.1462868241284589e-22, -1.4356250255311896e-23],
        [ 1.5077612072840900e-09, -2.5110829486803487e-13,  3.6604851278961747e-22,
          1.1462868241284589e-22,  8.7312038958868039e-07,  9.7945190701279520e-10],
        [-1.2011884887558450e-15, -1.2870493161486989e-13, -4.5428188275174624e-23,
         -1.4356250255311899e-23,  9.7945190701279562e-10,  9.3578059140706456e-06]
        ]),
    ...
    assert_close(
        obs['r44'],
        numpy.array(
        [[ 9.1104941877557704e-10, -3.0489009029304815e-15,  0.0000000000000000e+00,  0.0000000000000000e+00],
         [-3.0489009029453295e-15,  1.9135586753916078e-11,  0.0000000000000000e+00,  0.0000000000000000e+00],
    ...
    assert_close(
        obs['m66'],
        numpy.array(
        [[-7.3563108998434412e-01,  4.6737140210506745e+00,  0.0000000000000000e+00,
           0.0000000000000000e+00,  2.9985148734224228e-03, -6.2769167655290385e-07],
         [-9.8166216409446200e-02, -7.3566631849563779e-01,  0.0000000000000000e+00,
           0.0000000000000000e+00,  1.6901521608700989e-04, -3.5380663671177254e-08],
         [ 0.0000000000000000e+00,  0.0000000000000000e+00,  6.0980448553608135e-01,
          -2.0960292415929791e+00,  0.0000000000000000e+00,  0.0000000000000000e+00],
         [ 0.0000000000000000e+00,  0.0000000000000000e+00,  2.9967517970376734e-01,
           6.0980020978884764e-01,  0.0000000000000000e+00,  0.0000000000000000e+00],
         [ 1.2468790287015661e-06,  2.1544877348197787e-05,  0.0000000000000000e+00,
           0.0000000000000000e+00,  9.9998069084136043e-01, -2.0933014667636794e-04],
         [ 1.7009865738160104e-04,  2.9957968975905898e-03,  0.0000000000000000e+00,
           0.0000000000000000e+00,  2.2432585627026511e-03,  9.9999953040904210e-01]],
        ),
        atol=1E-12
    )
    ...
    assert_close(
        obs['orbit6'],
        [-2.6352679254087330e-09, -1.4584877433594875e-10,  0.0000000000000000e+00,
          0.0000000000000000e+00, -6.6969442416699896e-06, -5.8845211628962384e-02],
        atol=1E-20
    )

    assert_close(
        obs['emitXY'],
        [1.320358480897861e-10, 0.000000000000000e+00],
        rtol=1.e-9, atol=1e-18
    )
    assert_close(
        obs['emitXYZ'],
        [1.3222438725594531e-10, 2.4145091235719582e-37, 2.8584069315026826e-06],
        rtol=1.e-9, atol=1e-18
    )
