...
def test_linopt(dba_lattice, refpts):
    ...
    expected = np.array([[-0.616893565445970, 2.191800192047084, 0, 0],
    ...
    assert_close(
        obs['A'],
        np.array([[-0.616892545020345, 2.191796566512299],
                  [-0.282616790222590, -0.616892074542433]]),
        rtol=1e-7, atol=1e-12
    )
    assert_close(
        obs['B'],
        np.array([[-0.997384485665288, 0.466909288129080],
...
def test_nl_detuning_chromaticity(hmba_lattice):
    ...
    q0, q1, _, _, _, _ = at.nonlinear.detuning(hmba_lattice,
                                               npoints=11)
    assert_close(nlqplin, np.array([[0.2101570,  5.730634,  37.96993, -527.1578],
                           [0.3399707,  3.916998,  64.55810, -98.05048]]),
                 atol=1e-12, rtol=1e-5)
    assert_close(nlqpharm, np.array([[0.2101570,  5.730630,  37.96992, -527.1587],
                            [0.3399708,  3.916997,  64.55809, -98.05020]]),
                 atol=1e-12, rtol=1e-5)
    assert_close(q0, np.array([[0.210004, 0.340017], [0.210004, 0.340017]]),
                 atol=1e-12, rtol=1e-5)
    assert_close(q1, np.array([[96183.925683, -104218.18371],
...
[1.3203595267442241e-10, 1.1573828900796540e-37, 2.8586919200065274e-06],
    ...
    assert_close(
        obs['orbit6'],
        [-2.6352679435773159e-09, -1.4584877344363697e-10, 0.0000000000000000e+00,
         0.0000000000000000e+00, -6.6969442207863711e-06, -5.8845211247215208e-02],
        atol=1E-20
    )

    assert_close(
        obs['emitXY'],
        [1.320358475286751e-10, 0.000000000000000e+00],
        rtol=1.e-9, atol=1e-18
    )
    assert_close(
        obs['emitXYZ'],
        [1.3222438678441061e-10, 0.0000000000000000e+00, 2.8584082872712468e-06],
        rtol=1.e-9, atol=1e-18
    )
