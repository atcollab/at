...
def longres_object(srange, frequency, qfactor, rshunt, beta):
    ...
    """
    return res_object(srange, WakeComponent.Z, frequency, qfactor,
                      rshunt, beta)


def reswall_object(srange, wakecomp, length, rvac, conduct, beta,
                   yokoya_factor=1):
    """
    ...
    try:
        ...
        raise AtError('Wake object inputs should be either scalars '
                      'or with shape (len(wakecomp), )')
    for wc, le, rv, co, yk in zip(wakecomp, length, rvac, conduct,
                                  yokoya_factor):
