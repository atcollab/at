...
def get_survived(parts, grid, ring, nturns, method):
    """
    ...
    mask = numpy.isnan(pout[0,:,-1]) 
    survived = grid.grid[:,numpy.invert(mask)]   
    return survived, mask
...
def get_grid_boundary(mask, grid, config):
    ...
    def grid_boundary(mask, grid, config, nclean=2):
        ...
        mask2d = numpy.invert(numpy.reshape(mask.copy(), cnt))
...
def grid_boundary_search(ring, planes, npoints, amplitudes, nturns=1024,
    ...
    config = grid_configuration(planes, npoints, amplitudes, grid_mode, bounds=bounds)
    ...
    survived, mask = get_survived(parts, grid, newring, nturns, method)
...
def recursive_boundary_search(ring, planes, npoints, amplitudes, nturns=1024,
    ...
    def search_boundary(ring, planesi, angles, rtol, rstep, nturns, 
        ...
        while numpy.any(survived):
            ...
            _ = numpy.squeeze(method(ring, ptmp, nturns=nturns))
            survived[istracked] = numpy.invert(numpy.isnan(ptmp[0,:]))
        ...
        for a,stp in zip(angles, steps):
           for k, v in stp.items():
               ...
               if v: mask.append([x,y])
               grid.append([x,y])
        
        return numpy.squeeze(part[planesi]), numpy.array(mask).T, numpy.array(grid).T
    ...
    if use_mp:
        ...
        method = at.lattice_pass
    offset, newring = set_ring_orbit(ring, dp, refpts, offset)
    config = grid_configuration(planes, npoints, amplitudes, GridMode.RECURSIVE, bounds=bounds)
...
def boundary_search(ring, planes, npoints, amplitudes, nturns=1024,
    ...
    if grid_mode is GridMode.RECURSIVE:
        result = recursive_boundary_search(ring, planes, npoints, amplitudes, nturns=nturns,
                                         refpts=refpts, dp=dp, offset=offset, bounds=bounds,
                                         use_mp=use_mp, verbose=verbose)
    else:
        result = grid_boundary_search(ring, planes, npoints, amplitudes, nturns=nturns,
                                    refpts=refpts, dp=dp, offset=offset, bounds=bounds,
                                    grid_mode=grid_mode, use_mp=use_mp, verbose=verbose)
    return result
