...
def _element_from_m(line: str) -> Element:
    ...
    def makedir(mat_struct: Iterable[str]) -> Iterator[tuple[str, Any]]:
    ...
    def convert(value):
        ...
        if value.startswith("["):
            ...
            result = dict(makedir(argsplit(value[7:-1])))
        else:
            result = eval(value)
        return result
    ...
    kwargs = dict(zip(build_attrs, args, strict=True))
    kwargs.update(makedir(arguments[ll:]))
    kwargs["Class"] = matcls
    if matcls == "rbend":
...
def save_mat(ring: Lattice, filename: str | Path, **kwargs) -> None:
    ...
    lring = [el.to_file(encoder=_mat_encoder) for el in matlab_ring(ring)]
