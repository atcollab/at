...
void QuantDiffSeedPass(double *r_in, double* Lmatp , double *Seed, int nturn, int num_particles)
/* Lmatp 6x6 matrix
...
#define NUM_FIELDS_2_REMEMBER 2
{
    ...
    QuantDiffSeedPass(r_in, Lmatp, Seed, nturn, num_particles);
    return FieldNumbers;
}
...
void mexFunction(	int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    if (nrhs == 2) {
        ...
        r_in = mxGetPr(plhs[0]);
        QuantDiffSeedPass(r_in, Lmatp, Seed, 0, num_particles);
    }
    else if (nrhs == 0) {
