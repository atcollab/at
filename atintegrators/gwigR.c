...
static void GWigRadiationKicks(struct gwigR *pWig, double *X, double factorvect, double dl);
...
static void GWigPass_2nd(struct gwigR *pWig, double *X) 
{
  ...
  GWigAy(pWig, X, &ay, &aypx);
  GWigB(pWig, X, B);
  factorvect = vectorialnB2(pWig, X, B);
  X[1] -= ax;
  X[3] -= ay;
  
  GWigRadiationKicks(pWig, X, factorvect, dl);
  X[1] += ax;
  X[3] += ay;	 
 ...
 * Added by M. Borland, August 2007.
 * Modified by J. Arenillas, April 2024.
 */
{
  double dFactor, mpartialH;
  double ax, ay, axpy, aypx;
  double x ,xpr, y, ypr, p_norm;
  ...
  X[4] -= (pWig->srCoef)*dFactor*factorvect*mpartialH*dl;
  p_norm = 1/(1+X[4]);
...
b is the normalised magnetic field*/
	{
    double coef,gamma,E,P0;
	double v1x,v1y,v1z,vx,vy,vz,vmod;
	double bx,by,bz;
	
    E=(pWig->E0)*(1+X[4]);
	gamma=E/XMC2;
    vmod=clight*sqrt(1-1/(gamma*gamma));
    coef=(clight*sqrt(E*E-XMC2*XMC2))/(XMC2*gamma);
    v1x=X[1]*coef;
    v1y=X[3]*coef;
    v1z=sqrt(vmod*vmod-vx*vx-vy*vy);
	vx=v1x/vmod;
	vy=v1y/vmod;
	vz=v1z/vmod;
    ...
    return((SQR(bz*vy-by*vz) + SQR(bx*vz-bz*vx) + SQR(bx*vy-by*vx)));
}	
