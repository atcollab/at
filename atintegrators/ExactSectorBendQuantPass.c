...
static void ExactSectorBendQuant(
    double *r, double le, double irho, double *A, double *B, int max_order, int num_int_steps,
                                      ...
                                      double *r_in, int num_particles, struct parameters *Param)
{
    ...
    if (!Elem)
    {
        ...
        if (NumIntSteps == 0)
        {
            atError("NumIntSteps == 0 not allowed with radiation");
            check_error();
        }
    ...
    ExactSectorBendQuant(r_in, Elem->Length, irho,
...
MODULE_DEF(ExactSectorBendQuantPass) /* Dummy module initialisation */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    if (nrhs >= 2)
    {
        ...
        ExactSectorBendQuant(r_in, Length, irho, PolynomA, PolynomB,
