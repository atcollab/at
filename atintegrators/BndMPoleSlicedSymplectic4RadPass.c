...
void BndMPoleSymplectic4RadPass(double *r, double *le_array, double *irho_array, double *A_array, double *B_array,
        int max_order, int num_int_steps,
        double *entrance_angle_array, 	double *exit_angle_array,
        double fint1, double fint2, double gap,
        double *T1, double *T2,
        double *R1, double *R2, double E0, int num_particles, int Nslices)
        
        
{	int c, m, ii;
    double *r6, *A, *B;
    double SL, L1, L2, K1, K2, le, irho, entrance_angle, exit_angle;
    bool useT1, useT2, useR1, useR2, useFringe1, useFringe2;
    /*
   FILE *arxiu;
   
   
  arxiu=fopen("trackInside_slice.dat","w");
    ...
    {	r6 = r+c*6;
        if(!mxIsNaN(r6[0])) {
            ...
            for(ii = 0;ii<=(Nslices-1);ii++)	/* Loop over slices  */ {
                ...
                irho=irho_array[ii];
                le=le_array[ii];
                /* fprintf(arxiu,"%d %e %e %e %e %e\n",ii,B_array[ii*(max_order+1)],B[0],B[1],B[2],B[3]);
                fprintf(arxiu,"%d %e %e %e %e %e\n",ii,fint1,irho,entrance_angle,exit_angle,le);
                fprintf(arxiu,"%1.16e %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e\n",B_array[1],A_array[1],r6[0],r6[1],r6[2],r6[3],r6[4],r6[5],irho);*/
                SL = le/num_int_steps;
                L1 = SL*DRIFT1;
                L2 = SL*DRIFT2;
                K1 = SL*KICK1;
                ...
                /*
                fprintf(arxiu,"%1.16e %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e\n",B_array[1],A_array[1],r6[0],r6[1],r6[2],r6[3],r6[4],r6[5]);
                 */
                /* integrator */
                for(m=0; m < num_int_steps; m++) /* Loop over integration steps*/ {
                    r6 = r+c*6;
                    ATdrift6(r6, L1);
...
{	double *A , *B, *le, *ba, *irho, *entrance_angle, *exit_angle;
    double  *pr1, *pr2, *pt1, *pt2, fint1, fint2, gap;
    double E0;		/* Design energy [eV] */
    int ii, max_order, num_int_steps, Nslices;
    int *returnptr;
     ...
     */
        {
            ...
            ba = mxGetPr(mxGetFieldByNumber(ElemData, 0, fnum));
            ...
            entrance_angle = mxGetPr(mxGetFieldByNumber(ElemData, 0, fnum));
            ...
            exit_angle = mxGetPr(mxGetFieldByNumber(ElemData, 0, fnum));
        ...
        {	A = mxGetPr(mxGetFieldByNumber(ElemData, 0, FieldNumbers[0]));
            ...
            le = mxGetPr(mxGetFieldByNumber(ElemData, 0, FieldNumbers[4]));
            E0 = mxGetScalar(mxGetFieldByNumber(ElemData, 0, FieldNumbers[5]));
            ba = mxGetPr(mxGetFieldByNumber(ElemData, 0, FieldNumbers[6]));
            entrance_angle = mxGetPr(mxGetFieldByNumber(ElemData, 0, FieldNumbers[7]));
            exit_angle = mxGetPr(mxGetFieldByNumber(ElemData, 0, FieldNumbers[8]));
            Nslices = (int)mxGetScalar(mxGetFieldByNumber(ElemData, 0, FieldNumbers[9]));
            ...
            if(FieldNumbers[10]<0)
                ...
                fint1 = mxGetScalar(mxGetFieldByNumber(ElemData, 0, FieldNumbers[10]));
            
            
            if(FieldNumbers[11]<0)
                fint2 = 0;
            else
                fint2 = mxGetScalar(mxGetFieldByNumber(ElemData, 0, FieldNumbers[11]));
            
            if(FieldNumbers[12]<0)
                gap = 0;
            else
                gap = mxGetScalar(mxGetFieldByNumber(ElemData, 0, FieldNumbers[12]));
    ...
    irho=(double*)mxCalloc(Nslices, sizeof(double));
    for(ii=0;ii<=(Nslices-1);ii++) {
        irho[ii] = ba[ii]/le[ii];
    }
    
    BndMPoleSymplectic4RadPass(r_in, le, irho, A, B, max_order, num_int_steps,
            entrance_angle, exit_angle, fint1, fint2, gap, pt1, pt2, pr1, pr2, E0, num_particles, Nslices);
...
{	int m, n, ii;
    ...
    double *le, *ba, *A, *B;
    double *irho;
    int max_order, num_int_steps, Nslices;
    ...
    if(nrhs) {
        ...
        tmpmxptr = mxGetField(prhs[0], 0, "Lengths");
        if(tmpmxptr)
            le = mxGetPr(tmpmxptr);
        else
            mexErrMsgTxt("Required field 'Lengths' was not found in the element data structure");
        ...
        if(tmpmxptr)
            entrance_angle = mxGetPr(tmpmxptr);
        else
            mexErrMsgTxt("Required field 'EntranceAngle' was not found in the element data structure");
        ...
        r_in = mxGetPr(plhs[0]);
        BndMPoleSymplectic4RadPass(r_in, le, irho, A, B, max_order, num_int_steps,
                entrance_angle, exit_angle, fint1, fint2, gap, pt1, pt2, pr1, pr2, E0, n, Nslices);
        ...
        mxSetCell(plhs[0], 8, mxCreateString("Nslices"));
        mxSetCell(plhs[0], 9, mxCreateString("Energy"));
