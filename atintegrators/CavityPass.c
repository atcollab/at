...
void CavityPass(double *r_in, double le, double nv, double freq, double lag, double cavitylength, int num_particles)
/* le - physical length
...
{	int c, c6;
    ...
    {	for(c = 0;c<num_particles;c++)
        {	c6 = c*6;
            if(!atIsNaN(r_in[c6]))
                r_in[c6+5] += harm_number(cavitylength,freq)*C0/freq-cavitylength;
...
{
    if (!Elem) {
        double Length, Voltage, Energy, Frequency, TimeLag, CavityLength;
        ...
        CavityLength=atGetOptionalDouble(ElemData,"CavityLength",Param->CavityLength); check_error();
        ...
        Elem->CavityLength = CavityLength;
    }
    Param->CavityLength = 0.0;
    CavityPass(r_in,Elem->Length,Elem->Voltage/Elem->Energy,Elem->Frequency,
            Elem->TimeLag, Elem->CavityLength, num_particles);
    return Elem;
}
...
void mexFunction(	int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    if (nrhs == 2) {
        ...
        double CavityLength=atGetOptionalDouble(ElemData,"CavityLength",0);
        ...
        {   plhs[1] = mxCreateCellMatrix(2,1);
            ...
            mxSetCell(plhs[1],1,mxCreateString("CavityLength"));
        }
    }
	else {
