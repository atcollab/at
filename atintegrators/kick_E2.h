...
#ifdef DIFFUSION
#error "kick_E2 does not compute the diffusion matrix"
#endif
...
/* clang-format on */
    ...
    double K1 = (max_order >= 1) ? B[1] : 0.0;
    double K2 = (max_order >= 2) ? B[2] : 0.0;
    ...
    /* Multipole kick */
    r6[1] -= L * (ReSum - irho*dp_0 + irho*(irho*x + K1*(x*x-0.5*y*y) + K2*(x*x*x-4.0/3.0*x*y*y)));
    r6[3] += L * (ImSum + irho*(K1*x*y + 4.0/3.0*K2*x*x*y + (irho/6.0*K1-K2/3.0)*y*y*y)) ;
