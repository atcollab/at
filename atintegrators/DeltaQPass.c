...
void DeltaQPass(double *r_in,int num_particles,double alphax,double alphay,double betax,double betay,
                double qpx,double qpy,double a1,double a2, double a3){
    ...
    for(i=0;i<num_particles;i++){
        ...
        if(!mxIsNaN(rtmp[0])){
            ...
            r11x = cos(2*pi*dqx)+alphax*sin(2*pi*dqx);
            r12x = betax*sin(2*pi*dqx);
            r21x = -gx*sin(2*pi*dqx);
            r22x = cos(2*pi*dqx)-alphax*sin(2*pi*dqx);
            
            r11y = cos(2*pi*dqy)+alphay*sin(2*pi*dqy);
            r12y = betay*sin(2*pi*dqy);
            r21y = -gy*sin(2*pi*dqy);
            r22y = cos(2*pi*dqy)-alphay*sin(2*pi*dqy);
...
#define NUM_FIELDS_2_REMEMBER 9
{
	...
	switch(mode) {
	    ...
	    case USE_LOCAL_COPY:	/* Get fields from MATLAB using field numbers
            ...
            break;
	    default:
            mexErrMsgTxt("No match for calling mode in function slicer\n");
	}
       
	DeltaQPass(r_in, num_particles,alphax,alphay,betax,betay,qpx,qpy,a1,a2,a3);
	return FieldNumbers;
}
