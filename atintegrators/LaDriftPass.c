...
/*Large angle drift pass, 
 *Xiaobiao Huang, 8/22/2018
 */
void LaDriftPass(double *r_in, double le, int num_particles)
/* le - physical length
...
{	int c, c6;
	...
	for(c = 0;c<num_particles;c++)
		{	c6 = c*6;
		    if(!mxIsNaN(r_in[c6]))
			{    p_norm = 1./sqrt(SQR(1+r_in[c6+4])-SQR(r_in[c6+1])-SQR(r_in[c6+3])); 
			    NormL  = le*p_norm;
   			    r_in[c6+0]+= NormL*r_in[c6+1];
   			    r_in[c6+2]+= NormL*r_in[c6+3];
   			    r_in[c6+5]+= le*(p_norm*(1.0+r_in[c6+4]) -1.0);
   			 }
		}
}
...
{	double le;
	...
	LaDriftPass(r_in, le, num_particles);
	return(returnptr);
}
