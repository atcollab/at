...
%   Modified by L. Nadolski
    ...
    contentsfile = fullfile(folder,filename);
    if exist(contentsfile,'file')==2
        fid = fopen(contentsfile, 'r'); % open file reading
        h1 = fgetl(fid);
        header = h1(2:end);
        fclose(fid)
        delete(contentsfile)
    ...
    for d = 1:length(dirs)
        currentd = dirs{d};
        temp = getContents(currentd,'filter','*.m','sort',true);
        if ~isempty(temp)
            ...
            pkgprefix = strrep(currentd,[filesep '+'],'.');
            ...
            for f = 1:length(temp) % read H1 lines
                currentf = temp{f};
                H1_lines = [H1_lines; {get_H1_line([currentd filesep currentf])}]; %#ok<AGROW> % add H1 lines
                % remove extension from and add package prefix to m-files
                [~,fname,ext] = fileparts(currentf);
                if strcmpi(ext,'.m')
    ...
    % write to output
    nrows = length(files);
    fid = fopen(contentsfile, 'w'); % open file for writing
    fprintf(fid, '%%%s\n%%\n', header);
    ...
    fprintf(fid, '%%    \n%%   This file was generated by updateContents.m on %s.\n',datetime('now',format='dd MMM yyyy HH:mm:ss'));
    fclose(fid);
