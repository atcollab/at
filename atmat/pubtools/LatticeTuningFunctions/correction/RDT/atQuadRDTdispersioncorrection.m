function [rcor,inCOD,qs]=atQuadRDTdispersioncorrection(...
    rerr,...
    r0,...
    indBPM,...
    indQCor,...
...
if nargin<9
    if printouttext, disp(' --- wdisph=0.7 wtune=0.1'); end;
    wdisp=[.7 .1];
end
...
if isempty(ModelRM)
    ...
    if printouttext
        ...
        ModelRM=getresponsematrices(...
            ...
            [10 12]...        %8 specifiy rm to be computed
            );
...
[respqx,respqz]=qemrdtresp_mod(rerr,indBPM,indAllQuad);    % RDT response matrix assumes K=1
QL=atgetfieldvalues(rerr,indAllQuad,'Length');          % quadrupole lengths
...
[KQnoer,~,~]=EquivalentGradientsFromAlignments6D(r0,inCOD);
...
[KQi,~,~]=EquivalentGradientsFromAlignments6D(rerr,inCOD);
...
for iter=1:Niter
    ...
    [KQe,~,~]=EquivalentGradientsFromAlignments6D(rerr,inCOD);
    %KQ=atgetfieldvalues(rerr,indAllQuad,'PolynomB',{1,2});
    %KS=atgetfieldvalues(rerr,indAllSkew,'PolynomA',{1,2});
    ...
    rcor=rerr;
    rcor=atsetfieldvalues(rcor,indQCor,'PolynomB',{1,2},qs);
...
% get current rdt vectors to correct
[KQ,~,~]=EquivalentGradientsFromAlignments6D(rcor,inCOD);
