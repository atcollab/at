function [rcor,inCOD,qs,ss]=atRDTdispersioncorrection(...
    rerr,...
    r0,...
    indBPM,...
    indQCor,...
...
[KQi,KSi,~]=EquivalentGradientsFromAlignments6D(rerr,inCOD);
...
for iter=1:Niter
    ...
    [KQe,KSe,~]=EquivalentGradientsFromAlignments6D(rerr,inCOD);
    %KQ=atgetfieldvalues(rerr,indAllQuad,'PolynomB',{1,2});
    %KS=atgetfieldvalues(rerr,indAllSkew,'PolynomA',{1,2});
...
%     ylabel('Ver. disp.')
%     saveas(gca,['RDTdispCor' strrep(num2str(wdisp,'%2.2f_'),'.','p') '.fig']);
%     export_fig(['RDTdispCor' strrep(num2str(wdisp,'%2.2f_'),'.','p') '.jpg']);
