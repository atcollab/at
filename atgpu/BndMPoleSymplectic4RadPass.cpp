...
void BndMPoleSymplectic4RadPass::generateCode(std::string& code, PassMethodInfo *info,SymplecticIntegrator &integrator) noexcept {
  ...
  // Pure bend
  integrator.addDriftMethod("p_norm=1.0/(1.0 + r6[4]);fastdrift(r6,%STEP%,p_norm)");
  integrator.addKickMethod("bndthinkick0rad(r6,elem->PolynomA[0],elem->PolynomB[0],%STEP%,elem->irho,elem->CRAD,p_norm)");
  // DQ
  integrator.addDriftMethod("p_norm=1.0/(1.0 + r6[4]);fastdrift(r6,%STEP%,p_norm)");
  integrator.addKickMethod("dqthinkickrad(r6,elem->PolynomA[0],elem->PolynomB[0],elem->K,%STEP%,elem->irho,elem->CRAD,p_norm)");
